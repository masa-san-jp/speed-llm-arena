...
- あるエージェントについて、`parse_errors / calls` を集計する。**試合が有効かどうかで
  分母を絞らない**——無効になった試合の中の呼び出しも数える。判定したいのは
  「そのモデルが約束した形式で返せるか」であって、その試合が勝敗として成立したかではない。
  ウォームアップ失敗のように呼び出しが 0 の試合は、分母にも 0 しか足さないので影響しない。
- そのエージェントが**有効試合に** 10 試合以上参加しており、かつ parse エラー率が 1% を
  超える場合、`ranking[].ranking_valid` を `false` にする。試合数の側だけは有効試合で数える
  （少ない試合数で弾かないための下限なので、成立した試合で数えるのが筋）。
- 永続ランキング（§10）の `ranking_valid` も**同じ規則で計算する**。同じ名前の値が
  ファイルごとに違う意味を持つと、どちらが本当かを誰も言えなくなる。
...
- 山札のシードは試合インデックス（**0 始まり**）から決定的に生成し、再現可能にする。
  刻み幅はやり直しがあり得るかで変わる:

  | 走らせ方 | シード | なぜ |
  |---|---|---|
  | 総当たり（`--mode round-robin`） | `base_seed + 試合インデックス` | やり直しをしないので詰めて振れる |
  | はしご・永続ランキング（§10） | `base_seed + 試合インデックス * 2` | 無効試合のやり直しが `シード + 1` を使う（§10.3）。詰めて振ると、やり直しが起きた試合の後ろが全部ずれて再現できなくなる |

  はしご側でやり直しが起きなければ奇数側は使われないまま残る。**欠番は意図的なもの。**
...
`machine` には `display_name`、`chip`、`memory_gb` などの構成情報を任意で保存できる。`display_name` があれば Markdown の見出しに使い、同名のマシンを識別できるようにする。

`machine_id` はファイル名になるため、`[a-z0-9][a-z0-9._-]{0,63}` のみ許可する。`.` 始まり、`..`、`/` などは実行前に拒否する。

### 10.3 はしご方式と Elo

`--mode ladder` を既定とし、順位の正本は `players[]` の `rank` 順、Elo は併記値とする。Elo で並べ替えない。新入りは `lo=1, hi=N+1` から探索し、`N<=10` では `lo`、`N>=11` では `(lo+hi)//2` の順位に挑戦する。新入りの勝ちは `hi=相手の順位`、負けまたは引き分けは `lo=相手の順位+1` とし、`lo>=hi` で `lo` に挿入する。引き分けで新入りを相手より上に置かない。`N=0` は無試合で rank 1、`N=1` は通常探索する。
...
### 10.3.1 隣り合う順位の Elo 逆転を1試合で解消する

順位は rank 順が正本で Elo では並べ替えない（10.3）ため、`3位 Elo 969.5 / 4位 Elo 1016.0` のように、下の行のほうが Elo の高い並びが残ることがある。引き分けで下に入った選手は、番狂わせを起こすまで上がる機会がないためである。ルール上は正しいが、同じ表の2行を見比べる読み手にはそう読めない。

`--mode ladder` の実行の最後に、隣り合う2体だけを見て、下位のほうが Elo が高い組があればその2体で1試合する。下位が勝てば順位を入れ替え、負けまたは引き分けなら順位は動かさない。**1回の実行で解消するのは1組だけ**とし、対象が複数あるときは Elo 差が最大の組を選ぶ。連鎖して走る試合数が読めなくなるのを避けるためである。

この再戦は通常の試合として扱い、無効試合の扱い・やり直し・Elo の更新・母数への算入は既存の対戦と同じで、ランキングに反映する（10.4 の検証試合とは別物）。両者ともその実行に参加している場合にのみ行う。`--mode round-robin` では勝ち点で並べ直すため逆転が残らず、対象外とする。
